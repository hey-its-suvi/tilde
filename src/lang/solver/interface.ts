...
/** A circle. center is a point ref (key into model.points), not a literal point —
 *  this lets the existing point-placement machinery handle center placement. */
export type Circle = { center: string; r: Scalar }

/** Makes every field of T nullable. Used by working solver state to represent
 *  partial knowledge (e.g. a circle whose center is known but radius is not). */
export type Nullable<T> = T extends Scalar
  ? T | null
  : T extends string
  ? T | null
