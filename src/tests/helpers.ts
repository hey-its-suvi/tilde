...
export function assertCircleStatus(scene: SceneGraph, label: string, solutions: Solutions): void {
  const c = scene.circles.find(c => c.label === label)
  if (!c) {
    throw new Error(`assertCircleStatus: circle "${label}" not found\n  circles: [${scene.circles.map(c => c.label).join(', ')}]`)
  }
  if (c.solutions !== solutions) {
    throw new Error(`assertCircleStatus: "${label}" has solutions "${c.solutions}", expected "${solutions}"`)
  }
}
